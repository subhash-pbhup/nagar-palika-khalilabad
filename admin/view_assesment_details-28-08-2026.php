...
$floors = [];
$ward_display_value = '';
$mohalla_display_value = '';
$zone_display_value = '01';
$property_id_display = '';
...
if ($property_id > 0) {
    ...
    // Fetch Zone, Ward and Mohalla display values
    if ($property) {
        $zone_id = isset($property['zone_id']) && $property['zone_id'] !== ''
            ? (int)$property['zone_id'] : 1;
        $zone_display_value = str_pad((string)$zone_id, 2, '0', STR_PAD_LEFT);

        $ward_lookup_key = isset($property['ward_id']) && $property['ward_id'] !== ''
            ? (int)$property['ward_id']
            : (isset($property['ward']) && $property['ward'] !== '' ? (int)$property['ward'] : 0);

        if ($ward_lookup_key > 0) {
            ...
            if ($stmt_ward) {
                ...
                if ($ward_row = $ward_result->fetch_assoc()) {
                    $ward_display_value = htmlspecialchars($ward_row['ward_no'], ENT_QUOTES, 'UTF-8');
                } else {
                    $ward_display_value = htmlspecialchars((string)$ward_lookup_key, ENT_QUOTES, 'UTF-8');
                }
                $stmt_ward->close();
            }
        }

        $mohalla_lookup_key = isset($property['mohalla_id']) && $property['mohalla_id'] !== ''
            ? (int)$property['mohalla_id'] : 0;

        if ($mohalla_lookup_key > 0) {
            $stmt_mohalla = $conn->prepare("SELECT mohalla_name FROM mohalla WHERE mohalla_id = ? LIMIT 1");
            if ($stmt_mohalla) {
                $stmt_mohalla->bind_param("i", $mohalla_lookup_key);
                $stmt_mohalla->execute();
                $mohalla_result = $stmt_mohalla->get_result();
                if ($mohalla_row = $mohalla_result->fetch_assoc()) {
                    $mohalla_display_value = htmlspecialchars($mohalla_row['mohalla_name'], ENT_QUOTES, 'UTF-8');
                } else {
                    $mohalla_display_value = htmlspecialchars((string)$mohalla_lookup_key, ENT_QUOTES, 'UTF-8');
                }
                $stmt_mohalla->close();
            }
        }

        $property_id_display = !empty($property['property_id']) ? $property['property_id'] : 'N/A';
    }
...
// Keep database connection open because sidemenu.php uses it for permission checks.
...
<head>
    ...
    <title>Property Details - Khalilabad Nagar Palika</title>
            ...
            <h1 class="text-2xl font-bold text-gray-900">
                ...
                <?php if ($property): ?>
                    ...
                    <?php if (!empty($property['property_id'])): ?>
                        <span class="text-gray-500 ml-3 text-sm font-mono">
                            PID: <?php echo htmlspecialchars($property['property_id']); ?>
                        </span>
                    <?php endif; ?>
                <?php endif; ?>
            </h1>
            ...
            <div class="space-y-8">
                ...
                <section class="minimal-card p-6">
                    ...
                    <dl class="grid grid-cols-1 md:grid-cols-3 gap-y-6 gap-x-8 text-sm">
                        ...
                        <div class="data-item">
                            <dt>Zone</dt>
                            <dd><?php echo htmlspecialchars($zone_display_value); ?></dd>
                        </div>

                        <div class="data-item">
                            <dt>Ward No</dt>
                            <dd>
                                <?php echo $ward_display_value ?: 'N/A'; ?>
                                <?php if (!empty($property['ward_id'])): ?>
                                    <span class="text-xs text-gray-500 font-normal">
                                        (Ward ID: <?php echo (int)$property['ward_id']; ?>)
                                    </span>
                                <?php endif; ?>
                            </dd>
                        </div>
                        ...
                        <div class="data-item">
                            <dt>Mohalla Name</dt>
                            <dd>
                                <?php echo $mohalla_display_value ?: 'N/A'; ?>
                                <?php if (!empty($property['mohalla_id'])): ?>
                                    <span class="text-xs text-gray-500 font-normal">
                                        (Mohalla ID: <?php echo (int)$property['mohalla_id']; ?>)
                                    </span>
                                <?php endif; ?>
                            </dd>
                        </div>
                        ...
                        <div class="data-item">
                            <dt>Property ID</dt>
                            <dd class="font-mono tracking-wide text-emerald-700">
                                <?php echo htmlspecialchars($property_id_display); ?>
                            </dd>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="image-container">
                            <div class="data-item mb-2">
                                <dt>Center Image of Property</dt>
                            </div>
                            <?php
